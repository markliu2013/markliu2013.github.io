...
jQuery(function() {
    ...
    var sticky_navigation_offset_top = $('#main-custom-nav').offset().top-20;
         var sticky_navigation = function(){
        var scroll_top = $(window).scrollTop(); 
        if (scroll_top > sticky_navigation_offset_top) { 
            $('#main-custom-nav').css({'position': 'fixed', 'top' : '120px', 'margin-top' : '0px', 'right' : '80px'});
        } else {
            $('#main-custom-nav').css({ 'position': 'relative',  'max-width' : '100%', 'margin-top' : '170px', 'z-index' : '999' }); 
        }   
    };
    var offset_top = $(window).scrollTop(); 
    $(this).scrollTop(0);
	sticky_navigation();
	$(this).scrollTop(offset_top);
